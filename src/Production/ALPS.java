...
public class ALPS {
    ...
    String well_plate_id = "ALPS201903081";
...
//------------------------------------------------------- get raw n transfer---------------------------------------------------------------------------------
            ...
            String sql = "select c.julien_barcode , a.* from\n"
                    ...
                    + "(select pillar_plate_id from vibrant_test_tracking.pillar_plate_info where well_plate_id = '" + well_plate_id + "' and status ='finish_stitching') as b on a.pillar_plate_id = b.pillar_plate_id  left join\n"
            ...
            while (rs.next()) {
                ...
                String signal_type = pillar_plate_id.substring(3, 4).toUpperCase();
...
//---------------------------------------------------------------------UI-----------------------------------------------------------------------
    ...
    public void insertDB(HashMap<String, Double> dupMap, HashMap<String, Unit> map) {
        ...
        try {
            ...
            stmt.executeUpdate("UPDATE `vibrant_test_tracking`.`pillar_plate_info` SET `status`='finish' WHERE `pillar_plate_id`='" + map.get("A2A").pillar_plate_id + "';");
//            System.out.println("UPDATE `vibrant_test_tracking`.`pillar_plate_info` SET `status`='finish' WHERE `pillar_plate_id`='" + map.get("A2A").pillar_plate_id + "';");
            
            stmt.executeUpdate("UPDATE `vibrant_test_tracking`.`pillar_plate_info` SET `status`='finish' WHERE `pillar_plate_id`='" + map.get("A2G").pillar_plate_id + "';");
//            System.out.println("UPDATE `vibrant_test_tracking`.`pillar_plate_info` SET `status`='finish' WHERE `pillar_plate_id`='" + map.get("A2G").pillar_plate_id + "';");
            
            
            stmt.executeUpdate("insert into `tsp_test_qc_data`.`test_qc_data`(test_name,pillar_plate_id,cal_1,cal_2,cal_3,cal_4,pos_ctrl_1,neg_ctrl_1,time) values ('ALPSIGA','" + map.get("A2A").pillar_plate_id + "','100','50','25','12.5','62.5','1.5',now());");
            stmt.executeUpdate("insert into `tsp_test_qc_data`.`test_qc_data`(test_name,pillar_plate_id,cal_1,cal_2,cal_3,cal_4,pos_ctrl_1,neg_ctrl_1,time) values ('ALPSIGG_IGM','" + map.get("A2G").pillar_plate_id + "','100','50','25','12.5','62.5','1.5',now());;");
